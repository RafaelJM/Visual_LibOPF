    ...
    if (Q != NULL)
    {
        ...
        if ((Q->C.first != NULL) && (Q->C.last != NULL))
        {
            ...
            if (Q->L.elem != NULL)
            {
                ...
                Error(MSG1, "CreateGQueue"); return(0);
			}
        }
        else{
            Error(MSG1, "CreateGQueue"); return(0);
...
{
    GQueue *Q1 = CreateGQueue(nbuckets, (*Q)->L.nelems, (*Q)->L.value); if(errorOccurred) return(0);
    int i, bucket;
...
{
    RemoveGQueueElem(*Q, elem); if(errorOccurred) return;
    (*Q)->L.value[elem] = newvalue;
    InsertGQueue(Q, elem); if(errorOccurred) return;
}
