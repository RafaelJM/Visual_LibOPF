...
void opf_OPFTraining(Subgraph *sg)
{
  ...
  opf_MSTPrototypes(sg); if(errorOccurred) return;

  // initialization
  pathval = AllocFloatArray(sg->nnodes); if(errorOccurred) return;

  Q = CreateRealHeap(sg->nnodes, pathval); if(errorOccurred) return;
...
Subgraph *opf_OPFSemiLearning(Subgraph *sg, Subgraph *nonsg, Subgraph *sgeval)
{
  ...
  Subgraph *merged = opf_MergeSubgraph(sg, nonsg); if(errorOccurred) return(NULL);
  cont = 0;
  ...
  if (sgeval != NULL)
  {
    ...
    opf_OPFLearning(&merged, &sgeval); if(errorOccurred) return(NULL);

    for (i = 0; i < sg->nnodes; i++)
    {
      CopySNode(&sg->node[i], &merged->node[i], sg->nfeats); if(errorOccurred) return(NULL);
    }

    for (i = ((merged->nnodes) - (nonsg->nnodes)); i < merged->nnodes; i++)
    {
      CopySNode(&nonsg->node[cont], &merged->node[i], nonsg->nfeats); if(errorOccurred) return(NULL);
      cont++;
    }
  }

  // compute optimum prototypes
  opf_MSTPrototypes(sg); if(errorOccurred) return(NULL);

  merged = opf_MergeSubgraph(sg, nonsg); if(errorOccurred) return(NULL);

  // initialization
  pathval = AllocFloatArray(merged->nnodes); if(errorOccurred) return(NULL);

  Q = CreateRealHeap(merged->nnodes, pathval); if(errorOccurred) return(NULL);
...
{
  Train->bestk = opf_OPFknnLearning(Train, Eval, kmax); if(errorOccurred) return;
  opf_CreateArcs(Train, Train->bestk); if(errorOccurred) return;
  opf_PDF(Train); if(errorOccurred) return;
  opf_OPFClustering4SupervisedLearningForceOnePrototypePerClass(Train); if(errorOccurred) return;
...
int opf_OPFknnLearning(Subgraph *Train, Subgraph *Eval, int kmax)
{
  ...
  Subgraph *Train_cpy = CopySubgraph(Train), *Eval_cpy = CopySubgraph(Eval); if(errorOccurred) return(0);
  ...
  for (k = 1; k <= kmax; k++)
  {
    ...
    opf_PDF(Train_cpy); if(errorOccurred) return(0);
    opf_OPFClustering4SupervisedLearning(Train_cpy); if(errorOccurred) return(0);

    opf_OPFknnClassify(Train_cpy, Eval_cpy); if(errorOccurred) return(0);
    Acc = opf_Accuracy(Eval_cpy); if(errorOccurred) return(0);
    fprintf(stdout, " %.2f%%", Acc * 100);
...
{
  int i, j, k, l, knn = Train->bestk, *nn = AllocIntArray(knn + 1); if(errorOccurred) return;
  float dist = -1.0, *d = AllocFloatArray(Train->bestk + 1), tmp, cost, dens; if(errorOccurred) return;
  ...
  for (i = 0; i < sg->nnodes; i++)
  {
    ...
    while (adj_i != NULL)
    {
      ...
      if (sg->node[i].dens == sg->node[j].dens)
      {
        ...
        if (insert_i){
          InsertSet(&(sg->node[j].adj), i); if(errorOccurred) return;
		}
      }
      adj_i = adj_i->next;
    }
...
Set *opf_OPFClustering4ANN(Subgraph *sg)
{
  ...
  for (i = 0; i < sg->nnodes; i++)
  {
    ...
    while (adj_i != NULL)
    {
      ...
      if (sg->node[i].dens == sg->node[j].dens)
      {
        ...
        if (insert_i){
          InsertSet(&(sg->node[j].adj), i); if(errorOccurred) return(NULL);
		}
      }
      adj_i = adj_i->next;
    }
  ...
  pathval = AllocFloatArray(sg->nnodes); if(errorOccurred) return(NULL);
  Q = CreateRealHeap(sg->nnodes, pathval); if(errorOccurred) return(NULL);
  SetRemovalPolicyRealHeap(Q, MAXVALUE);
  ...
  while (!IsEmptyRealHeap(Q))
  {
    ...
    if (sg->node[p].pred == NIL)
    {
      InsertSet(&prototypes, p); if(errorOccurred) return(NULL);
      pathval[p] = sg->node[p].dens;
      sg->node[p].label = l;
      l++;
    }
...
void opf_RemoveIrrelevantNodes(Subgraph **sg)
{
  ...
  {
    newsg = CreateSubgraph((*sg)->nnodes - num_of_irrelevants); if(errorOccurred) return;
    ...
    for (i = 0; i < (*sg)->nnodes; i++)
    {
      if ((*sg)->node[i].relevant)
      { // relevant node
        CopySNode(&(newsg->node[k]), &((*sg)->node[i]), newsg->nfeats); if(errorOccurred) return;
        k++;
      }
    }
    newsg->nlabels = (*sg)->nlabels;
...
void opf_MoveIrrelevantNodes(Subgraph **src, Subgraph **dst)
{
  ...
  if (num_of_irrelevants > 0)
  {
    newsrc = CreateSubgraph((*src)->nnodes - num_of_irrelevants); if(errorOccurred) return;
    newdst = CreateSubgraph((*dst)->nnodes + num_of_irrelevants); if(errorOccurred) return;
    ...
    for (i = 0; i < (*dst)->nnodes; i++){
      CopySNode(&(newdst->node[i]), &((*dst)->node[i]), newdst->nfeats); if(errorOccurred) return;
    }
	j = i;
    ...
    for (i = 0; i < (*src)->nnodes; i++)
    {
      if ((*src)->node[i].relevant){ // relevant node
        CopySNode(&(newsrc->node[k++]), &((*src)->node[i]), newsrc->nfeats); if(errorOccurred) return;
	  }
	  else{
        CopySNode(&(newdst->node[j++]), &((*src)->node[i]), newdst->nfeats); if(errorOccurred) return;
	  }
	}
    DestroySubgraph(&(*src));
    DestroySubgraph(&(*dst));
...
void opf_MoveMisclassifiedNodes(Subgraph **src, Subgraph **dst, int *p)
{
  ...
  *p = num_of_misclassified;
  ...
  {
    newsrc = CreateSubgraph((*src)->nnodes - num_of_misclassified); if(errorOccurred) return;
    newdst = CreateSubgraph((*dst)->nnodes + num_of_misclassified); if(errorOccurred) return;
...
Subgraph *opf_ReadModelFile(char *file)
{
  ...
  if (fread(&nnodes, sizeof(int), 1, fp) != 1){
    Error("Could not read number of nodes", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }

  g = CreateSubgraph(nnodes); if(errorOccurred) return(NULL);
  if (fread(&g->nlabels, sizeof(int), 1, fp) != 1){
    Error("Could not read number of labels", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }
  if (fread(&g->nfeats, sizeof(int), 1, fp) != 1){
    Error("Could not read number of features", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }

  /* for supervised opf by pdf */
  if (fread(&g->df, sizeof(float), 1, fp) != 1){
    Error("Could not read adjacency radius", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }
  if (fread(&g->bestk, sizeof(int), 1, fp) != 1){
    Error("Could not read the best k", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }
  if (fread(&g->K, sizeof(float), 1, fp) != 1){
    Error("Could not read K", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }
  if (fread(&g->mindens, sizeof(float), 1, fp) != 1){
    Error("Could not read minimum density", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }
  if (fread(&g->maxdens, sizeof(float), 1, fp) != 1){
    Error("Could not read maximum density", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
  }
  ...
  for (i = 0; i < g->nnodes; i++)
  {
    ...
    if (fread(&g->node[i].position, sizeof(int), 1, fp) != 1){
      Error("Could not read node position", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}
    if (fread(&g->node[i].truelabel, sizeof(int), 1, fp) != 1){
      Error("Could not read node true label", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}
    if (fread(&g->node[i].pred, sizeof(int), 1, fp) != 1){
      Error("Could not read node predecessor", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}
    if (fread(&g->node[i].label, sizeof(int), 1, fp) != 1){
      Error("Could not read node label", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}
    if (fread(&g->node[i].pathval, sizeof(float), 1, fp) != 1){
      Error("Could not read node path value", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}
    if (fread(&g->node[i].radius, sizeof(float), 1, fp) != 1){
      Error("Could not read node adjacency radius", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}
    if (fread(&g->node[i].dens, sizeof(float), 1, fp) != 1){
      Error("Could not read node density value", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}

    for (j = 0; j < g->nfeats; j++)
      if (fread(&g->node[i].feat[j], sizeof(float), 1, fp) != 1){
        Error("Could not read node features", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	  }
  }

  for (i = 0; i < g->nnodes; i++)
    if (fread(&g->ordered_list_of_nodes[i], sizeof(int), 1, fp) != 1){
      Error("Could not read ordered list of nodes", "opf_ReadModelFile"); if(errorOccurred) return(NULL);
	}
...
Subgraph **kFoldSubgraph(Subgraph *sg, int k)
{
  ...
  for (i = 0; i < k - 1; i++)
  {
    out[i] = CreateSubgraph(foldsize); if(errorOccurred) return(NULL);
  ...
  out[i] = CreateSubgraph(foldsize + totelems); if(errorOccurred) return(NULL);
  out[i]->nfeats = sg->nfeats;
  out[i]->nlabels = sg->nlabels;
...
{
  int *label = AllocIntArray(sg->nlabels + 1), i, j, i1, i2; if(errorOccurred) return;
  int *nelems = AllocIntArray(sg->nlabels + 1), totelems; if(errorOccurred) return;
  srand((int)time(NULL));
  ...
  *sg1 = CreateSubgraph(totelems); if(errorOccurred) return;
  *sg2 = CreateSubgraph(sg->nnodes - totelems); if(errorOccurred) return;
  (*sg1)->nfeats = sg->nfeats;
  (*sg2)->nfeats = sg->nfeats;

  for (i1 = 0; i1 < (*sg1)->nnodes; i1++){
    (*sg1)->node[i1].feat = AllocFloatArray((*sg1)->nfeats); if(errorOccurred) return;
  }
  for (i2 = 0; i2 < (*sg2)->nnodes; i2++){
    (*sg2)->node[i2].feat = AllocFloatArray((*sg2)->nfeats); if(errorOccurred) return;
  }
...
Subgraph *opf_MergeSubgraph(Subgraph *sg1, Subgraph *sg2)
{
  if (sg1->nfeats != sg2->nfeats){
    Error("Invalid number of feats!", "MergeSubgraph"); if(errorOccurred) return(NULL);
  }

  Subgraph *out = CreateSubgraph(sg1->nnodes + sg2->nnodes); if(errorOccurred) return(NULL);
  int i = 0, j;
  ...
  for (i = 0; i < sg1->nnodes; i++){
    CopySNode(&out->node[i], &sg1->node[i], out->nfeats); if(errorOccurred) return(NULL);
  }
  for (j = 0; j < sg2->nnodes; j++)
  {
    CopySNode(&out->node[i], &sg2->node[j], out->nfeats); if(errorOccurred) return(NULL);
    i++;
  }
...
float opf_Accuracy(Subgraph *sg)
{
  ...
  for (i = 0; i <= sg->nlabels; i++){
    error_matrix[i] = (float *)calloc(2, sizeof(float)); if(errorOccurred) return(0);
  }

  nclass = AllocIntArray(sg->nlabels + 1); if(errorOccurred) return(0);
...
float *opf_Accuracy4Label(Subgraph *sg){
	...
	nclass = AllocIntArray(sg->nlabels+1); if(errorOccurred) return;
...
float *opf_Accuracy4Label(Subgraph *sg)
{
  ...
  nclass = AllocIntArray(sg->nlabels + 1); if(errorOccurred) return(NULL);
  error = AllocFloatArray(sg->nlabels + 1); if(errorOccurred) return(NULL);
...
float **opf_ReadDistances(char *fileName, int *n)
{
  ...
  if (fread(&nsamples, sizeof(int), 1, fp) != 1){
    Error("Could not read number of samples", "opf_ReadDistances"); return(NULL);
  }
  ...
  *n = nsamples;
  ...
  for (i = 0; i < nsamples; i++)
  {
    ...
    {
      Error("Could not read samples", "opf_ReadDistances"); return(NULL);
    }
  }
  fclose(fp);
...
float opf_NormalizedCut(Subgraph *sg)
{
  ...
  ncut = 0.0;
  acumIC = AllocFloatArray(sg->nlabels); if(errorOccurred) return(0);
  acumEC = AllocFloatArray(sg->nlabels); if(errorOccurred) return(0);
...
void opf_BestkMinCut(Subgraph *sg, int kmin, int kmax)
{
  ...
  float *maxdists = opf_CreateArcs2(sg, kmax); if(errorOccurred) return; // stores the maximum distances for every k=1,2,...,kmax
  ...
  for (k = kmin; (k <= kmax) && (mincut != 0.0); k++)
  {
    ...
    opf_PDFtoKmax(sg); if(errorOccurred) return;

    opf_OPFClusteringToKmax(sg); if(errorOccurred) return;

    nc = opf_NormalizedCutToKmax(sg); if(errorOccurred) return;
  ...
  fprintf(stdout, "Best k: %d ", sg->bestk);
}
...
void opf_CreateArcs(Subgraph *sg, int knn)
{
  int i, j, l, k;
  float dist;
  int *nn = AllocIntArray(knn + 1); if(errorOccurred) return;
  float *d = AllocFloatArray(knn + 1); if(errorOccurred) return;
  ...
  for (i = 0; i < sg->nnodes; i++)
  {
    ...
    for (l = 0; l < knn; l++)
    {
      if (d[l] != INT_MAX)
      {
        ...
        InsertSet(&(sg->node[i].adj), nn[l]); if(errorOccurred) return;
      }
    }
  }
  free(d);
...
void opf_PDF(Subgraph *sg)
{
  int i, nelems;
  float dist;
  float *value = AllocFloatArray(sg->nnodes); if(errorOccurred) return;
  Set *adj = NULL;
...
void opf_ElimMaxBelowArea(Subgraph *sg, int A)
{
  ...
  area = SgAreaOpen(sg, A); if(errorOccurred) return;
  for (i = 0; i < sg->nnodes; i++)
  {
...
void opf_ElimMaxBelowVolume(Subgraph *sg, int V)
{
  ...
  volume = SgVolumeOpen(sg, V); if(errorOccurred) return;
  for (i = 0; i < sg->nnodes; i++)
  {
...
float *opf_CreateArcs2(Subgraph *sg, int kmax)
{
  int i, j, l, k;
  float dist;
  int *nn = AllocIntArray(kmax + 1); if(errorOccurred) return(NULL);
  float *d = AllocFloatArray(kmax + 1); if(errorOccurred) return(NULL);
  float *maxdists = AllocFloatArray(kmax); if(errorOccurred) return(NULL);
  /* Create graph with the knn-nearest neighbors */
  ...
  for (i = 0; i < sg->nnodes; i++)
  {
    ...
    for (l = kmax - 1; l >= 0; l--)
    {
      if (d[l] != FLT_MAX)
      {
        ...
        InsertSet(&(sg->node[i].adj), nn[l]); if(errorOccurred) return(NULL);
      }
    }
  }
  free(d);
...
void opf_OPFPruning(Subgraph **gTrain, Subgraph **gEval, float desiredAcc)
{
  ...
  while ((t <= max_iterations) && (fabs(currentAcc - oldAcc) <= desiredAcc))
  {
    ...
    opf_OPFClassifying(*gTrain, *gEval);
    currentAcc = opf_Accuracy(*gEval); if(errorOccurred) return;
    fprintf(stdout, "Current accuracy: %.2f%% ", currentAcc * 100);
    t++;
    fprintf(stdout, "OK");
  }
}