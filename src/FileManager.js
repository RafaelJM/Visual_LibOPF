export let Module = require('./libopf.js')(); // Your Emscripten JS output file
export let FS = Module.FS;
...
export function readGraph(dv, title, description){
  var subGraph= {
  ...
  for(var i = 0; i < subGraph.nnodes; i++){
    ...
    subGraph.nodes[i].color = colors[subGraph.nodes[i].truelabel]
  ...
  subGraph.children = subGraph.nodes; //temp, se existir ponteiro seria bom
  return(subGraph);
}

export function writeGraph(subGraph, file){
...
export function readModelFile(dv, description, title){
  var subGraph= {
...
export function writeModelFile(subGraph, file){
...
export function readClassification(file, title, description){
  return({classification: FS.readFile(file,{encoding: 'utf8'}).split("\n"), title: title, description:description})
}

export function writeClassification(classification, file){
  var aux = ""
...
export function readDistances(dv, title, description){
  var cont = 0;
...
export function writeDistances(distances, file){
